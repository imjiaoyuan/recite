...
const STRINGS: Record<string, Dict> = {
  zh: {
    ...
    'sync.key': '同步码',
    'sync.keyPh': '自定一个，各设备填同一个码',
    'sync.keyBad': '同步码只能用字母、数字、-、_，长度 4–64',
    'sync.token': '访问令牌（可选）',
    'sync.tokenPh': '多人共用同一服务时设置',
    'sync.now': '立即同步',
    'sync.syncing': '同步中…',
    'sync.ok': '✓ 已同步',
    'sync.pulled': '✓ 已同步，从云端拉取了 {n} 个词的进度',
    'sync.fail': '同步失败：{msg}',
    'sync.v': 'v{v} · 更新内容见 README',
    'sync.never': '从未同步',
    'sync.last': '上次同步 {time}',
    'sync.sameOrigin': '✓ 本部署自带数据存储，只需填同步码',
    'sync.descHosted': '学习数据存在你部署的 KV 存储里，各设备用同一同步码读写。',
    'sync.deploy': '如何部署？',
  ...
  en: {
    ...
    'sync.key': 'Sync code',
    'sync.keyPh': 'Pick your own — same code on every device',
    'sync.keyBad': 'Letters, digits, - and _ only; 4–64 characters',
    'sync.token': 'Access token (optional)',
    'sync.tokenPh': 'Set when sharing one service with others',
    'sync.now': 'Sync now',
    'sync.syncing': 'Syncing…',
    'sync.ok': '✓ Synced',
    'sync.pulled': '✓ Synced — pulled {n} words from the cloud',
    'sync.v': 'v{v} · see README for changes',
    'sync.fail': 'Sync failed: {msg}',
    'sync.never': 'Never synced',
    'sync.last': 'Last sync {time}',
    'sync.sameOrigin': '✓ This deployment has its own data store — only the sync code is needed',
    'sync.descHosted': 'Learning data lives in your deployment’s KV storage; devices read/write it with the same sync code.',
