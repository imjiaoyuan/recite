// Dictation: listen to the pronunciation, type the word. No meaning shown until reveal.
import { h } from '../ui';
...
export default function dictation([listId]: string[], { navigate }: Ctx): ViewResult {
  ...
  function drawDone(): void {
    ...
    area.append(
      h('div', { class: 'done' },
        ...
        h('div', {},
          h('button', { class: 'btn primary', onclick: () => navigate(`#/dictation/${listId}`) }, t('done.again')),
  ...
  function render(): void {
    ...
    const card = h('div', { class: 'entry dict-card' });
    if (timerEl) card.append(timerEl);
    card.append(
